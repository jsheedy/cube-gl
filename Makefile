...
cube-geometry-shader: osc.hpp osc.cpp cube-geometry-shader.cpp shader.h shaders/brain-glow.vs shaders/brain-glow.fs shaders/geometry.vs shaders/geometry.fs shaders/geometry.gs
	g++ -std=c++11 `pkg-config --cflags liblo` `pkg-config --libs liblo` `pkg-config --cflags glfw3` -o cube-geometry-shader cube-geometry-shader.cpp osc.cpp glad.c  `pkg-config --static --libs glfw3` -I .
